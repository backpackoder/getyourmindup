...
import { Alert, AlertTitle, Box, Button, Checkbox, Chip, Grid, TextField, Typography } from "@mui/material";
...
export function Thank() {
  ...
  return (
    ...
    >
      ...
      <form onSubmit={handleSubmit(onCreatePublication)} noValidate>
        <Grid container spacing={2} alignContent={'center'} justifyContent={'center'}>
          ...
          <Grid item xs={12} textAlign={'center'} >
            ...
            <Chip label={isPrivate ? 'Private thank you post' : 'Public thank you post'}
              clickable
              color={isPrivate ? 'primary' : "warning"}
              variant="outlined"
              onClick={() => setIsPrivate(!isPrivate)}
              className="fadeIn" />
            <Checkbox color={isPrivate ? 'primary' : "warning"} checked={isPrivate} onChange={(e, idChecked) => setIsPrivate(idChecked)} icon={<LockOpenIcon />} checkedIcon={<LockIcon />} />
          </Grid>
      ...
      <Box sx={{ position: { xs: 'relative', md: 'absolute' }, top: { xs: 10, md: 90 }, left: { md: 28 } }} className="fadeIn">

        {thanks?.map(thank => (
          <Alert key={thank} icon={<GradeIcon sx={{ color: yellow[400] }} />} sx={{ borderRadius: '4% / 50%', backgroundColor: blue[100], color: 'black', mb: 2 }} elevation={1} >
            <AlertTitle></AlertTitle>
            {thank} - <strong>A person in the world</strong>
          </Alert>
        ))
        }
        <Alert icon={<GradeIcon sx={{ color: yellow[400] }} />} sx={{ borderRadius: '4% / 50%', backgroundColor: blue[100], color: 'black', mb: 2 }} elevation={1} >
          I thank you for the beautiful day today - <strong>A person in the world</strong>
        </Alert>
        <Alert icon={<GradeIcon sx={{ color: yellow[400] }} />} sx={{ borderRadius: '20px', backgroundColor: blue[100], color: 'black', mb: 2 }} elevation={1} >
          I thank you for the beautiful day today - <strong>A person in the world</strong>
        </Alert>
        <Alert icon={<GradeIcon sx={{ color: yellow[400] }} />} sx={{ borderRadius: '20px', backgroundColor: blue[100], color: 'black', mb: 2 }} elevation={1} >
          I thank you for the beautiful day today - <strong>A person in the world</strong>
